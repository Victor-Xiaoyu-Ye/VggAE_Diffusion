...
本版独立实现新 DiT/trainer，不基于25/26，也不复制V-RAE setting。目标是首帧条件下联合生成其余8帧；默认先用历史 `r7_t2_c192_v2/joint/checkpoint_best.pt`，另用 `AE_VARIANT=t1geo112` 比较。它是待验证的生成基线，尚无910B训练或视频质量结果。
...
WINDOW_NAMESPACE=r7_window_t2v2_x0_diag_v1 \
bash /cache/yexiaoyu/VggAE_Diffusion/scripts/scale/30_run_window_baseline.sh
```

30依次执行新28缓存(train/eval)、merge和新29训练；缓存已有完整标记可跳过。默认1个缓存partition覆盖48卡，train9935/eval64由实际OFT文件清单决定。缓存为新namespace，单独编码首帧条件，eval额外存完整原片。不会复用旧n1训练或重复未来帧。
...
首次上910B建议保持同一模型、完整未来窗口和6000步scheduler，先计划暂停在50步，检查真实前后向、AE回放、采样、双写与耗时：
...
恢复时除 `STOP_AFTER_STEPS` 和输入/输出定位外保持模型、目标、归一化、数据、精度、batch、world size、scheduler总预算和评估配置一致。不同实验用新namespace：例如 `AE_VARIANT=t1geo112 WINDOW_NAMESPACE=r7_window_t1geo112_x0_diag_v1`，或 `PREDICTION=velocity WINDOW_NAMESPACE=r7_window_t2v2_velocity_diag_v1`。
...
910B算子与BF16、HCCL、真实AE回放、OBS权限/网络吞吐、48卡显存和自由采样质量仍待集群验证。不能据这些CPU测试宣布能生成合理视频。先看真实AE回放是否维持原有重建，再看500/1000等固定留出样本是否有内容、条件一致性和合理运动；不以单一去噪loss或低运动的“稳定”替代质量。
