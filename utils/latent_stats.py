...
def create_moments(num_frames, latent_dim, device):
    accumulation_dtype = (
        torch.float32 if device.type == "npu" else torch.float64)
    return {
        "sum": torch.zeros(
            num_frames, latent_dim, device=device,
            dtype=accumulation_dtype),
        "sum_sq": torch.zeros(
            num_frames, latent_dim, device=device,
            dtype=accumulation_dtype),
        "count": torch.zeros(
            num_frames, 1, device=device,
            dtype=accumulation_dtype),
    }
...
def update_moments(moments, tensor):
    ...
    batch_sum = values.sum(dim=(0, 2))
    batch_sum_sq = values.square().sum(dim=(0, 2))
    moments["sum"] += batch_sum.to(
        device=moments["sum"].device, dtype=moments["sum"].dtype)
    moments["sum_sq"] += batch_sum_sq.to(
        device=moments["sum_sq"].device, dtype=moments["sum_sq"].dtype)
    moments["count"] += values.shape[0] * values.shape[2]


def merge_raw_moments(entries):
    if not entries:
        raise ValueError("Cannot merge an empty raw-moment list")
    return {
        key: sum(entry[key].double().cpu() for entry in entries)
