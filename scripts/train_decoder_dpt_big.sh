#!/bin/bash
# Experiment A: DPTHead + capacity (features=512) + multi-scale + 200 epochs
set -e
...
ASCEND_RT_VISIBLE_DEVICES=${GPU_IDS} torchrun --nproc_per_node=${NUM_GPUS} --master_port=29504 \
  ${PROJECT}/train_decoder_dpt.py \
    ...
    --output_dir ${PROJECT}/ckpts/decoder_dpt/exp-6-big \
    --seq_len 8 \
    --batch_size 4 --accum_steps 4 \
    --epochs 200 --lr 1e-4 --wd 1e-2 \
    --features 512 --multi_scale \
