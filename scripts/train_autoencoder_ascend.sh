#!/bin/bash
# Phase 1 (Ascend 910B): Train Generative Tokenizer A + Compact Decoder G
set -e
...
ASCEND_RT_VISIBLE_DEVICES=${GPU_IDS} torchrun --nproc_per_node=${NUM_GPUS} --master_port=29510 \
  ${PROJECT}/train_autoencoder.py \
    ...
    --output_dir ${PROJECT}/ckpts/autoencoder/exp-1-ascend \
    --latent_dim 512 --latent_grid 18 \
    --levels 4 11 17 23 \
    --decoder_base_dim 256 --decoder_num_resblocks 1 \
    --decoder_version v1 \
    --latent_noise_std 0.05 --latent_noise_warmup 1000 \
    --lambda_l1 1.0 --lambda_grad 0.05 --lambda_temporal 0.05 --lambda_latent_reg 0.01 \
