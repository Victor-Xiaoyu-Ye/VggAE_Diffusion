#!/bin/bash
# exp-10: DPTHead + multi-layer mean → boundary level only, others keep original
#         (RAEv2 mean applied only to level 11, preserving multi-scale diversity)
set -e
...
CUDA_VISIBLE_DEVICES=${GPU_IDS} torchrun --nproc_per_node=${NUM_GPUS} --master_port=29514 \
  ${PROJECT}/train_decoder_dpt.py \
    ...
    --output_dir ${PROJECT}/ckpts/decoder_dpt/exp-10-mean-bdry \
    --seq_len 8 \
    --batch_size 10 --accum_steps 4 \
    --epochs 120 --lr 1e-4 --wd 1e-2 \
    --features 256 --multi_layer_mean --mlm_boundary_only \
