...
"${PYTHON_BIN}" "${PROJECT}/scripts/prepare_domain_csv.py" --metadata "${SPATIALVID_METADATA}" \
  --selection "${PROJECT}/configs/spatialvid_domain_v1.json" --output "${DOMAIN_DIR}"
...
export R7_CACHE_VERSION="r7_domain_${ARM}_t2v2_legacy_diag_v1"
export DIAGNOSTIC_SOURCE="${DIAGNOSTIC_SOURCE:-r7_domain_${ARM}_uniform_v1}"
export WINDOW_NAMESPACE="${WINDOW_NAMESPACE:-r7_domain_${ARM}_other_s6000_v1}"
