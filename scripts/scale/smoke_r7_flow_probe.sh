...
PYTHONPATH="${PROJECT}" "${PYTHON_BIN}" "${PROJECT}/scripts/test_output_snapshot.py"
PYTHONPATH="${PROJECT}" "${PYTHON_BIN}" "${PROJECT}/scripts/test_flow_run_status.py"
PYTHONPATH="${PROJECT}" "${PYTHON_BIN}" "${PROJECT}/scripts/test_r7_flow_probe.py"
export STAGE=smoke FLOW_NAMESPACE="${FLOW_NAMESPACE:-r7_geo112_flow_smoke_probe_v2}"
export DIAGNOSTIC_SAMPLE_STEPS='' ORACLE_STARTS='' DIAGNOSTIC_EVERY=1
unset RESUME
...
for kind in plain_x0 preconditioned direct_velocity; do
