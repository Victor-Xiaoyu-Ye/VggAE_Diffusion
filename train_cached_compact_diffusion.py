...
def save_checkpoint(path, model, ema, optimizer, scheduler, step, stats, args):
    atomic_torch_save({
...
def save_latent_preview(target_raw, generated_raw, path, latent_grid):
    ...
    combined = torch.cat([target_map.flatten(), generated_map.flatten()])
    low = torch.quantile(combined, 0.02)
    high = torch.quantile(combined, 0.98)
...
def save_rgb_preview(target_raw, generated_raw, cond_raw, rgb_preview,
                     device, model_dtype, path, latent_grid, latent_dim):
...
def evaluate_preview(model, preview_batch, target_mean, target_std,
    ...
    x0 = torch.randn(
        target.shape, device=device, dtype=model_dtype,
        generator=generator)
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Cached diffusion training requires CUDA")
    ...
    while global_step < args.max_steps:
        ...
        if is_main_process() and save_due:
            save_checkpoint(
                ...
                base_model, ema, optimizer, scheduler, global_step,
                cache_stats, args)
    ...
    if is_main_process():
        save_checkpoint(
            ...
            ema, optimizer, scheduler, global_step, cache_stats, args)
