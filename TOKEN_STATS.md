...
Each cache rank reduces a batch on the NPU, transfers only the small `[S,D]`
batch sums to CPU, and accumulates FP64 `sum`, `sum_sq`, and `count` there.
Rank-local moments are uploaded beside the OBS shards; global rank 0 combines
them before producing FP32 mean/std for training. This avoids unsupported NPU
FP64 while also avoiding precision loss from billions of FP32 additions.
Legacy partitions containing only mean/std remain readable, but a merge
refuses to mix raw-moment and legacy partitions.
